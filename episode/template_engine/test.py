...
class OeyeTest(TestCase):
    ...
    def try_render(self, text, ctx=None, result=None):
        ...
        """
        actual = Oeye(text).render(ctx or {})
        if result:
            self.assertEqual(actual, result)

    def assertSynErr(self, msg):
        pat = "^" + re.escape(msg) + "$"
        return self.assertRaisesRegex(OeyeSyntaxError, pat)

    def test_passthrough(self):
        # Strings without variables are passed through unchanged.
        self.assertEqual(Oeye("Hello").render(), "Hello")
        self.assertEqual(Oeye("Hello, 20% fun time!").render(), "Hello, 20% fun time!")
    ...
    def test_reusability(self):
        # A single Oeye can be used more than once with different data.
        globs = {
            "upper": lambda x: x.upper(),
            "punct": "!",
        }

        template = Oeye("This is {{name|upper}}{{punct}}", globs)
